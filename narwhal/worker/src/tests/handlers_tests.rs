...
async fn synchronize_versioned_batches() {
    ...
    mock_server
        .expect_request_batch()
        .withf(move |request| request.body().batch == digest_v1)
        .times(2)
        .returning(move |_| {
            Ok(anemo::Response::new(RequestBatchResponse {
                batch: Some(mock_batch_response_v1.clone()),
            }))
        });
    let mock_batch_response_v2 = batch_v2.clone();
    mock_server
        .expect_request_batch()
        .withf(move |request| request.body().batch == digest_v2)
        .times(2)
        .returning(move |_| {
            Ok(anemo::Response::new(RequestBatchResponse {
                batch: Some(mock_batch_response_v2.clone()),
            }))
        });
    let routes = anemo::Router::new().add_rpc_service(WorkerToWorkerServer::new(mock_server));
    let target_worker = target_primary.worker(id);
